...
function LevelFour({onGoHome,timeSpent,comp}) {
 ...
 const [finished, setFinished] = useState(false);
  ...
  useEffect(() => {
    setLetters(shuffle(TRICK_LETTERS).slice(0, 3));
  }, []);
  ...
  const check = () => {
    ...
    if (ok) {
      setScore((s) => s + 1);
      setBgColor("from-emerald-400 to-green-500");
      say("Perfect!");
      setTimeout(() => setIdx((k) => k + 1), 600);
    } else {
      setShowError(true);
      setBgColor("from-red-400 to-red-600");
...
// bg-gradient-to-r ${bgColor}
  return (
    <div className={`h-[calc(100vh-56px)] px-4 pb-6 bg-gradient-to-r ${bgColor} transition-all duration-500`}>
      {/* HEADER */}
      <div className="sticky top-0 z-20 flex items-center justify-between p-4 mb-4 bg-white/70 backdrop-blur-lg rounded-xl shadow-md">
      ...
      {showError && (
        <div className="hidden lg:block absolute top-20 left-[40%] -translate-x-1/2 bg-red-500 text-white px-6 py-4 rounded-2xl shadow-lg font-bold text-xl animate-bounce z-[5]">
