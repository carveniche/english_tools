export default function DailyLimitCard({
  showingDailyLimitRef,
  showingAllCardLimitRef,
}) {
  const isDailyLimit = showingDailyLimitRef?.current;
  const isAllCardLimit = showingAllCardLimitRef?.current;

  // console.log(isDailyLimit, "daily", isAllCardLimit, "all");

  return (
    <div className="bg-white text-center p-[1rem]
      flex justify-center absolute items-center w-full h-full left-0 top-[0] flex-col gap-[1rem]">
   ...
   <div className="flex justify-center">
      ...
      <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
        {isDailyLimit ? "Daily limit is over" : isAllCardLimit? "No data available":''}
      </h1>

      <p className="text-sm md:text-base text-gray-500">
        {/* {isDailyLimit
          ? "You’ve reached today’s limit. Come back tomorrow to continue."
          : ""} */}
           {isDailyLimit ? "You’ve reached today’s limit. Come back tomorrow to continue." : isAllCardLimit? "No data available":''}
      </p>
    </div>
  );
}
