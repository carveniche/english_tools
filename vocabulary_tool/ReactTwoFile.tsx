...
export type QMatch = QBase & { type: 'match'; pairs: { word: string; meaning: string }[] };
...
function buildMatchFrom(words: VocabItem[]): QMatch {
  ...
  const pairs = shuffledWords.map(w => ({ word: w.word, meaning: w.meaning || '' }));
  return {
...
function QMatchView({ q, onCorrect, onWrong }: { q: QMatch; onCorrect: () => void; onWrong: () => void }) {
  ...
  const [wrong, setWrong] = useState(false);
  const hasCheckedRef = useRef(false);
  const allWords = q.pairs.map(p => p.word);
  const availableWords = shuffle(allWords.filter(w => !Object.values(matches).includes(w)));
      ...
      <div className='space-y-3'>
        {q.pairs.map(p => (
          <div key={p.meaning} className='flex items-center gap-3'>
            <div className='flex-1 bg-white/10 p-2 rounded text-white'>{p.meaning}</div>
            <div
